// file-input.tsx
import { useState } from "react";import { IoCloudUploadOutline } from "react-icons/io5";
...
const FileInput: React.FC<FileInputProps> = ({ setFormData }) => {
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      formData.append("images", selectedFile); // Append the file to FormData with the key 'images'
      setFormData(formData); // Update the state with the FormData
    } else {
      setFormData(null); // Clear the FormData if no file is selected
    }
  };
